function [ConvertedData,ConvertVer,ChanNames,GroupNames,ci]=tdms2mat_helper(varargin)
...
%
% [ConvertedData,ConvertVer,ChanNames]=tdms2mat_helper(SaveConvertedFile,filename)
%
%       Inputs:
...
%         end
        ...
        if kTocDaqMxRawData
            error(sprintf(['\n Seqment %.0f of the above file contains data in the DAQmxRaw NI datatype format which is not supported '...
                'with this function. See help documentation in tdms2mat.m for how to fix this. '],segCnt),'DAQmxRawData Format Not Supported');
            fclose(fid);
        end
